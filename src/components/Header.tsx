        ...
        <nav className="hidden lg:flex items-center gap-4 text-xs font-medium overflow-x-auto no-scrollbar [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`transition-colors whitespace-nowrap py-1 ${pathname === link.href ? 'text-sky-400 border-b-2 border-sky-400 font-bold' : 'text-slate-400 hover:text-white'}`}
            >
              {link.label}
            </Link>
          ))}
  ...
  return (
    <footer className="bg-[#090D16] border-t border-slate-800 py-12 text-slate-400 text-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 pt-6 border-t border-slate-900 text-center text-slate-500 text-[11px]">
        © 2026 CarePlus Medical Center. All rights reserved.
      </div>
